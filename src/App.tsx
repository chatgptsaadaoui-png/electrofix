...
export default function App() {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        try {
          ...
          errorDetail = json.details || json.error || text;
        } catch (e) {
  ...
  const handleAddCustomer = async (e: React.FormEvent) => {
    ...
    if (!newCustomer.name.trim() && !newCustomer.phone.trim() && !newCustomer.address.trim()) {
      alert('يرجى ملء خانة واحدة على الأقل');
      return;
    }
  ...
  const loadInitialData = async () => {
    ...
    try {
      ...
      setServerError(error.message || "فشل تحميل البيانات الأولية");
    } finally {
  ...
  const handleUpdateRepairStatus = async (id: number, status: string) => {
    try {
      ...
      console.error('Error in handleUpdateRepairStatus:', error);
      alert('حدث خطأ أثناء تحديث حالة الإصلاح: ' + error.message);
    }
  };
